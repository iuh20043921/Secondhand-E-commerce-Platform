...
$orderCheck = $conn->prepare("SELECT MaNguoiMua FROM donhang WHERE MaDonHang = ? AND MaNguoiMua = ?");
$orderCheck->bind_param("ii", $orderId, $userId);
$orderCheck->execute();
$orderCheck->get_result();


$sql = "SELECT ct.*, sp.TenSP, sp.HinhSP ,TrangThai
        from donhang dh 
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <td><img src="../images/<?php echo htmlspecialchars($item['HinhSP']); ?>" alt="<?php echo htmlspecialchars($item['TenSP']); ?>" style="width: 50px; height: 50px;"></td>
                ...
                <td> <?php if ($item['TrangThai'] == 'Đã giao'): ?>
                        <a href="danhgia.php?madon=<?php echo $orderId; ?>&masp=<?php echo $item['MaSP']; ?>">
                            <button class="btn btn-sm btn-success">Đánh giá</button>
                        </a>
                    <?php endif; ?></td>
            </tr>
            </tr>
